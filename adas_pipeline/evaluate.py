...
def main():
    ...
    logger.info(f"Operating threshold (from model): {thr:.3f}")
    preds = (probs >= thr).astype(int)
    logger.info("─" * 56)
    logger.info(f"  Samples  : {len(probs)}  (+ve {int(labels.sum())})")
    logger.info(f"  Accuracy : {accuracy_score(labels, preds):.4f}")
    logger.info(f"  Precision: {precision_score(labels, preds, zero_division=0):.4f}")
    logger.info(f"  Recall   : {recall_score(labels, preds, zero_division=0):.4f}")
    logger.info(f"  F1       : {f1_score(labels, preds, zero_division=0):.4f}")
    if len(np.unique(labels)) > 1:
        logger.info(f"  ROC-AUC  : {roc_auc_score(labels, probs):.4f}")
        logger.info(f"  Avg-Prec : {average_precision_score(labels, probs):.4f}")
